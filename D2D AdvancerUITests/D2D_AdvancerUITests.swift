...
final class D2D_AdvancerUITests: XCTestCase {
    ...
    private func assertPushedHeaderTitleCentered(
        ...
        let screenCenterX = appFrame.midX

        XCTAssertGreaterThanOrEqual(
            titleFrame.minX,
            backFrame.maxX + 8,
            "\(screenName) centered header title overlaps the back button. title=\(titleFrame), back=\(backFrame)"
        )
        XCTAssertEqual(
            titleFrame.midX,
            screenCenterX,
            accuracy: 8,
            "\(screenName) header title should be centered on the screen. title=\(titleFrame), screen=\(appFrame)"
        )
    }
