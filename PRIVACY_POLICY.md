...
When you choose **More > Apple Contacts > Scan Contacts**, the app checks accessible contact name, company, department, job-title, and, when Apple permits note access, contact-note fields on your device for the service phrases shown in the import screen. For matching contacts, the app also reads the available phone, email, and postal-address fields needed to prepare a lead. A matching contact's note is shown in the review screen and copied into the lead notes field only when you select and import that contact. Nothing is copied into D2D Advancer until you review and select a match. Imported contacts become personal workspace leads and follow the same local and optional iCloud storage rules described below.
